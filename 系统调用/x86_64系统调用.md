...
```
其中`__USER32_CS`为用户态Segment Selector，`__KERNEL_CS`为内核态Segment Selector。`__USER32_CS`宏引用了GDT_ENTRY_DEFAULT_USER32_CS宏，该宏是用户态代码段在GDT（[Global Descriptor Table](https://en.wikipedia.org/wiki/Global_Descriptor_Table)）中的索引。`__KERNEL_CS`引用了GDT_ENTRY_KERNEL_CS宏，该宏是内核态代码段在GDT中的索引。
